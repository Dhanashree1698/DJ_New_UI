...
// }
  ...
  GetFilterData() { 
    debugger
    const apiUrl = this._global.baseAPIUrl + 'SearchFileStatus/GetFileStorageSearchDataByFilter'; 
...
//this.modalRef.hide
  ...
  OnLoad()
  {
    this.getSearchRecordsResult(0);
  }

  getSearchRecordsResult(tempID:any) {
  debugger
   const apiUrl = this._global.baseAPIUrl + 'api/SearchFileStatus/getSearchDataByFolderStructure?UserID=' + localStorage.getItem('UserID') + '&user_Token=' + localStorage.getItem('User_Token') + '&TemplateID=' + 0  
   this._onlineExamService.getAllData(apiUrl).subscribe((data: [any]) => {
         this._FileList = data;
         this._FilteredList = data;
